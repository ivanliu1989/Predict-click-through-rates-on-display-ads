...
def get_x(csv_row, D):
    ...
    for key, value in csv_row.items():
        index = mmh3.hash((value + key[1:]),42) % D  # weakest hash ever ;)
        x.append(index)
...
def get_p(x, w, ld):
    ...
    return (1.+ld) / (1. + exp(-max(min(wTx, 20.), -20.))) # bounded sigmoid
...
with open('submissionPython22Sep2014_pm.csv', 'w') as submission:
